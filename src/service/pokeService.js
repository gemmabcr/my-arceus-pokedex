...
export class PokeService {
  ...
  async getPokemons () {
    ...
    const newData = json.pokemon_entries.map(pokemonEntry => {
      return {
  ...
  async getTypePokemon (url) {
    const data = await this.sendRequest(url)
    const spanishType = data.names.filter(item => item.language.name === 'es')
    return {
      name: spanishType[0].name,
      image: `https://rerollcdn.com/ARCEUS/Types/${capitalize(data.name)}.svg`
    }
  }
